# promptcue | Demo run across all 12 query types with full enrichment
# Maintainer: Informity
#
# Exercises every query type in a single pass with language detection,
# linguistic extraction, and keyword extraction all enabled.  Prints a
# formatted summary for each query showing classification result, routing
# hints, action hints, and extracted features.
#
# Run with:
...
# ==============================================================================
# Test corpus — one representative query per type
# ==============================================================================
...
def _row(label: str, value: object) -> None:
    """Print a single labelled output row, left-aligned to WIDTH."""
    print(f'  {label:<{WIDTH}} {value}')


def main() -> None:
    """Run all 12 query types through PromptCue and print a summary for each."""
    analyzer = PromptCueAnalyzer(PromptCueConfig(
        enable_language_detection    = True,
        enable_linguistic_extraction = True,
        enable_keyword_extraction    = True,
    ))
    analyzer.warm_up()  # pre-load all models; avoids cold-start latency on the first query

    print(f'\n{"PromptCue Demo Run":^{WIDTH + 50}}')
    print(f'{"— all 12 query types, full enrichment enabled —":^{WIDTH + 50}}\n')
    ...
    for expected_type, query in QUERIES:
        ...
        label = f'{result.primary_query_type}  {match}  (expected: {expected_type})'
        _row('primary_query_type',  label)
